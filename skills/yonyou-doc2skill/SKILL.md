...
- 输入：文档站、仓库、PDF、Wiki、iKM 知识地图、FAQ、chat 记录、交付资料
...
- 如果用户给的是 iKM 知识地图链接，优先要求 `cookie` 或环境变量 `IKM_COOKIE`，并使用 `python3 scripts/run.py ikm --mode map ...`
- 如果用户要按部门门户/栏目批量沉淀 iKM，使用 `python3 scripts/run.py ikm --mode portal ...`
- 如果用户要按主题关键词批量沉淀 iKM，使用 `python3 scripts/run.py ikm --mode search --keyword ...`
- 如果 iKM 知识主要在附件里，默认追加 `--parse-attachments`，把 PDF、Word、PPT、文本等附件解析进 `references/content.md`
...
python3 scripts/run.py ikm --mode map --pk MAP_ID --actionlocid PORTAL_ID --parse-attachments --name ikm-knowledge-map
python3 scripts/run.py ikm --mode portal --actionlocid PORTAL_ID --parse-attachments --name ikm-portal-knowledge
python3 scripts/run.py ikm --mode search --keyword YonLinker --actionlocid PORTAL_ID --parse-attachments --name ikm-yonlinker-search
python3 scripts/run.py chat --export-path /path/to/slack-export --platform slack --name team-chat
