...
1. **当前 agent 接手模式**，适合 Codex、OpenClaw、Claude Code 这类对话式宿主
   - 先执行 `yonyou-doc2skill enhance <skill_dir> --mode prepare`
   - 这个命令只生成 `.enhance/` 增强上下文包
   - 然后由当前对话 agent 读取 `.enhance/` 并重写 `SKILL.md`
   - 这种模式最容易贴合当前会话里的真实目标
2. **脚本自己闭环增强**，适合纯 CLI 场景
   - `--target claude|gemini|openai` 走 API 模式
   - `--agent codex|claude|copilot|opencode|kimi` 走 LOCAL 模式

```bash
# 推荐：准备增强上下文，再让当前 agent 接手
...
# 在对话式宿主里，优先先生成增强上下文包，再让当前 agent 接手
yonyou-doc2skill enhance output/react/ --mode prepare

# 或尝试 LOCAL 模式（使用本地 Codex / Claude / 其他 agent CLI）
yonyou-doc2skill enhance output/react/ --agent codex
...
### 截图 OCR 脱敏效果不理想？
```bash
# 中文截图、流程图、交付资料默认建议这样跑
yonyou-doc2skill sanitize-assets apply ./delivery-assets \
  --output output/sanitized \
  --ocr-engine auto
```

- `auto` 会优先使用 `rapidocr`，其次 `paddleocr`，最后回退到 `tesseract`
- 当前最推荐的中文截图 OCR 是 `rapidocr`
- 真实项目样本验证里，`rapidocr` 对流程图、组织图、业务截图的中文识别明显优于 `tesseract`
- Logo 脱敏仍建议走 `logo-scan` + 模板匹配，不要只依赖 OCR
...
| OCR 脱敏（rapidocr）| 1–10 秒/图 | 中文截图推荐 |
| 打包 | 5–10 秒 | 最终 .zip 创建 |
